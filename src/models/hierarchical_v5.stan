...
model {
  ...
  mu_beta      ~ normal(0, 1);
  tau_beta     ~ cauchy(0, 1);
  beta         ~ normal(mu_alpha, tau_alpha);

  lambda_sigma ~ gamma(2,2);
  sigma        ~ exponential(lambda_sigma);

  for (i in 1:N)
    bill_length[i] ~ normal(alpha[group[i]] + beta[group[i]] * bill_depth[i], sigma[group[i]]);
}
...
generated quantities {
  ...
  vector[N] log_lik;
  for (i in 1:N) {
    real mu_i  = alpha[group[i]] + beta[group[i]] * bill_depth[i];
    yrep[i]    = normal_rng(mu_i, sigma[group[i]]);
    log_lik[i] = normal_lpdf(bill_length[i] | mu_i, sigma[group[i]]);
  }
}
